...
function init() {
	...
	// Re-use chessGameService's wallet so all coordinator txs share one nonce queue
	const wallet = chessGameService.getWallet();
	if (!wallet) {
		console.warn("[Escrow] Coordinator wallet not available — escrow disabled");
		return;
	}
	contract = new ethers.Contract(ESCROW_ADDRESS, ESCROW_ABI, wallet);
 ...
 * Routes through chessGameService's enqueue so all coordinator txs
 * share a single nonce-managed queue.
 *
...
async function resolveMatch(gameCode, winner) {
	...
	const receipt = await chessGameService.enqueue((n) =>
		contract.resolveMatch(gameCodeBytes32, winner, { nonce: n })
	).then((tx) => tx.wait());
	return receipt;
}
